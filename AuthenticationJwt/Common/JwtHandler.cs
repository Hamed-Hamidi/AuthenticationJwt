...
namespace AuthenticationJwt.Common
{
    public class JwtHandler : IJwtHandler
    {
        ...
        public JsonWebToken CreateToken(string userId, string role,IDictionary<string, string> claims = null , UserScope userScope = null)
        {
            ...
            {
                AccessToken = token,
                RefreshToken = string.Empty,//generate new token and insert to db by user Id for client call refresh token before expire token
                Expires = expires.ToTimestamp(),
                Id = userId,
                Role = role ?? string.Empty, // pass user scope from controller after login
                Claims = customClaims.ToDictionary(c => c.Type, c => c.Value),
                UserScope = null // pass user scope from controller after login
           };
           return jsonWebToken;
        }
